function [] = plot_CLCD(exp_value, sel_speed, div, chord, kin_viscosity, plot_type, format)
...
%         end
        ...
        for k = 1:length(exp_value.f_avg)
             ...
             if (exp_value.vel(k) == sel_speed(j))
                if exist('k1','var') == 0
                    errorbar(exp_value.aoa(k), (-exp_value.f_ratio(k) / div(1, j)), exp_value.f_std_ratio(k) / div(1, j), 'ob', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                else
                    errorbar(exp_value.aoa(k), (-exp_value.f_ratio(k) / div(1, j)), exp_value.f_std_ratio(k) / div(1, j), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                    x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
                    y_vec = [(-exp_value.f_ratio(k1) / div(1, j)), (-exp_value.f_ratio(k) / div(1, j))];
                    plot(x_vec, y_vec, '--b', 'HandleVisibility','off')
                end
                k1 = k;
             end
