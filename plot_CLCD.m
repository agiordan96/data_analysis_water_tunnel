function [] = plot_CLCD(wingtype, sel_inflation, exp_value, sel_speed, chord, kin_viscosity, plot_type, format)
  ...
  elseif wingtype == "soft"
    ...
    for k = 1:length(exp_value.f_avg)
        if (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(1))
            if exist('k1','var') == 0
                        errorbar(exp_value.aoa(k), -exp_value.f_ratio(k), exp_value.f_std_ratio(k), 'ob', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'red')
                    else
                        errorbar(exp_value.aoa(k), -exp_value.f_ratio(k), exp_value.f_std_ratio(k), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'red')
                        x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
                        y_vec = [-exp_value.f_ratio(k1), -exp_value.f_ratio(k)];
                        plot(x_vec, y_vec, '--b', 'HandleVisibility','off')
             end
