    ...
    elseif wingtype == "soft_hard"
        ...
        for j = 1:length(sel_speed) % looping over flow speed to create fixed-speed plots
            ...
            for k = 1:length(exp_value.f_avg)
                ...
                if k <= length(exp_value_hard.vel)
                    if (exp_value_hard.vel(k) == sel_speed(j))
                        if exist('hard1','var') == 0
                            errorbar(exp_value_hard.aoa(k), (-exp_value_hard.f_avg(k, lift_dir) / div(1, j)), exp_value_hard.f_std(k, lift_dir) / div(1, j), 'o', 'Color', '#383B3E', 'DisplayName', '(solid) rigid', 'CapSize', 18, 'LineWidth', 1, MarkerFaceColor='#383B3E')
                        else
                ...
                if sel_speed(j) == double_vel 
                      ...
                      if exp_value_double.inflation(k) == sel_inflation_double(1)
                       if exp_value_double.dir(k) == 1
                            if exist('d1','var') == 0
                                errorbar(exp_value_double.aoa(k), (-exp_value_double.f_avg(k, lift_dir) / div(1, j)), exp_value_double.f_std(k, lift_dir) / div(1, j), 'o', 'Color', '#383B3E', 'DisplayName', '(dotted) neutral', 'CapSize', 18,  'LineWidth', 1, MarkerFaceColor = '#383B3E', MarkerEdgeColor = '#383B3E')
                            else
                ...
                if (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(1))
                    if exist('k1','var') == 0
                        errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir) / div(1, j)), exp_value.f_std(k, lift_dir) / div(1, j), 'o', 'Color', '#383B3E', 'DisplayName', '(dotted) neutral', 'CapSize', 18,  'LineWidth', 1, MarkerFaceColor = '#383B3E', MarkerEdgeColor = '#383B3E')
                    else
