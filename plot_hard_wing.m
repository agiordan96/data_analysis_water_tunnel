   ...
   if ~isfolder('..')
    ...
    for j = 1:length(sel_speed) % looping over flow speed to create fixed-speed plots
        ...
        for k = 1:length(exp_value.f_avg)
             ...
             if (exp_value.vel(k) == sel_speed(j))
                if exist('k1','var') == 0
                    errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, 2) / div(1, j)), exp_value.f_std(k, 2) / div(1, j), 'ob', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                else
                    errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, 2) / div(1, j)), exp_value.f_std(k, 2) / div(1, j), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                    x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
    ...
    for j = 1:length(sel_speed) % looping over flow speed to create fixed-speed plots
        ...
        for k = 1:length(exp_value.f_avg)
             ...
             if (exp_value.vel(k) == sel_speed(j)) 
                if exist('k1','var') == 0
                    errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, 1) / div(1, j)), exp_value.f_std(k) / div(1, j), 'ob', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                else
                    errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, 1) / div(1, j)), exp_value.f_std(k) / div(1, j), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                    x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
