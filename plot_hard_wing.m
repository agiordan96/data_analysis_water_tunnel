    ...
    for j = 1:length(sel_speed) % looping over flow speed to create fixed-speed plots
        ...
        for k = 1:length(exp_value.f_avg)
             if (exp_value.vel(k) == sel_speed(j))
                 ...
                 if exist('k1','var') == 0
                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, lift_dir) / exp_value.f_avg(k, drag_dir), exp_value.f_std(k, 2) / exp_value.f_std(k, 1), 'ob', 'DisplayName', 'inf. = 120 mL', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                 else
                    errorbar(exp_value.aoa(k), exp_value.f_avg(k, lift_dir) / exp_value.f_avg(k, drag_dir), exp_value.f_std(k, 2) / exp_value.f_std(k, 1), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                    x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
                    y_vec = [exp_value.f_avg(k1, 2) / exp_value.f_avg(k1, 1), exp_value.f_avg(k, lift_dir) / exp_value.f_avg(k, drag_dir)];
...
%% CD / CL: presenting one plot per selected speed and all inflations, against AoA
    ...
    % % 
    % %             if exist('k1','var') == 0
    % %             errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'or', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'r', 'LineWidth', 1, MarkerEdgeColor = 'red')
    % %             else
    % %                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'or', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'r', 'LineWidth', 1, MarkerEdgeColor = 'red')
    % %                 x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
    % %                 y_vec = [exp_value.f_avg(k1, 1) / exp_value.f_avg(k1, 2), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir)];
    ...
    % %              end
    % %              if exist('k2','var') == 0
    % %              errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'ok', 'DisplayName', 'inf. = 60 mL', 'CapSize', 18, 'MarkerFaceColor', 'k', 'LineWidth', 1, MarkerEdgeColor = 'black')
    % % 
    % %              else
    % %                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 2) / exp_value.f_std(k, 1), 'ok', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'k', 'LineWidth', 1, MarkerEdgeColor = 'black')
    ...
    % %              end
    % %              if exist('k3','var') == 0
    % %              errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'om', 'DisplayName', 'inf. = 90 mL', 'CapSize', 18, 'MarkerFaceColor', 'm', 'LineWidth', 1, MarkerEdgeColor = 'magenta')
    % %              else
    % %                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'om', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'm', 'LineWidth', 1, MarkerEdgeColor = 'magenta')
    ...
    % %              end
    % %              if exist('k4','var') == 0
    % %              errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'ob', 'DisplayName', 'inf. = 120 mL', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
    % %              else
    % %                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
    ...
    % %              end
    % %              if exist('k5','var') == 0
    % %             errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'og', 'DisplayName', 'inf. = 30 mL', 'CapSize', 18, 'MarkerFaceColor', 'g', 'LineWidth', 1, MarkerEdgeColor = 'green')
    % %             else
    % %                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, drag_dir) / exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 1) / exp_value.f_std(k, 2), 'og', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'green')
   ...
   if ~isfolder('..')
    ...
    for j = 1:length(sel_speed) % looping over flow speed to create fixed-speed plots
        ...
        for k = 1:length(exp_value.f_avg)
             ...
             if (exp_value.vel(k) == sel_speed(j))
                if exist('k1','var') == 0
                    errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir) / div(1, j)), exp_value.f_std(k, 2) / div(1, j), 'ob', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                else
                    errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir) / div(1, j)), exp_value.f_std(k, 2) / div(1, j), 'ob', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
                    x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
                    y_vec = [(-exp_value.f_avg(k1, 2) / div(1, j)), (-exp_value.f_avg(k, lift_dir) / div(1, j))];
                    plot(x_vec, y_vec, '--b', 'HandleVisibility','off')
                end
                k1 = k;
             end
        ...
        for k = 1:length(exp_value.f_avg)
             ...
             if (exp_value.vel(k) == sel_speed(j)) 
                if exist('k1','var') == 0
                    ...
                    x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
                    y_vec = [(-exp_value.f_avg(k1, 1) / div(1, j)), (-exp_value.f_avg(k, drag_dir) / div(1, j))];
                    plot(x_vec, y_vec, '--b', 'HandleVisibility','off')
                end
                k1 = k;
             end
...
%% Lift: presenting one plot per selected speed against AoA
    ...
    %          if (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(1))
    %             if exist('k1','var') == 0
    %                 errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'or', 'DisplayName', 'neutral', 'CapSize', 18, 'MarkerFaceColor', 'r', 'LineWidth', 1, MarkerEdgeColor = 'red')
    %             else
    %                 errorbar(exp_value.aoa(k), -exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 2), 'or', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'r', 'LineWidth', 1, MarkerEdgeColor = 'red')
    %                 x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
    %                 y_vec = [-exp_value.f_avg(k1, 2)  , (-exp_value.f_avg(k, lift_dir))];
    %                 plot(x_vec, y_vec, '--r', 'HandleVisibility','off')
    %             end
    %             k1 = k;
    %          elseif (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(2))
    %              if exist('k2','var') == 0
    %                 errorbar(exp_value.aoa(k), (exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'ok', 'DisplayName', 'inf. = 60 mL', 'CapSize', 18, 'MarkerFaceColor', 'k', 'LineWidth', 1, MarkerEdgeColor = 'black')
    %              else
    %                 errorbar(exp_value.aoa(k), exp_value.f_avg(k, lift_dir), exp_value.f_std(k, 2), 'ok', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'k', 'LineWidth', 1, MarkerEdgeColor = 'black')
    %                 x_vec = [exp_value.aoa(k2), exp_value.aoa(k)];
    ...
    %          elseif (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(3))
    %              if exist('k3','var') == 0
    %                 errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'om', 'DisplayName', 'inf. = 90 mL', 'CapSize', 18, 'MarkerFaceColor', 'm', 'LineWidth', 1, MarkerEdgeColor = 'magenta')
    %              else
    %                 errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'om', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'm', 'LineWidth', 1, MarkerEdgeColor = 'magenta')
    %                 x_vec = [exp_value.aoa(k3), exp_value.aoa(k)];
    ...
    %          elseif (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(4))
    %              if exist('k4','var') == 0
    %                 errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'ob', 'DisplayName', 'inf. = 120 mL', 'CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
    %              else
    %                 errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'ob', 'HandleVisibility','off','CapSize', 18, 'MarkerFaceColor', 'b', 'LineWidth', 1, MarkerEdgeColor = 'blue')
    %                 x_vec = [exp_value.aoa(k4), exp_value.aoa(k)];
    ...
    %          elseif (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(5))
    %             if exist('k5','var') == 0 
    %              errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'og', 'DisplayName', 'inf. = 30 mL', 'CapSize', 18, 'MarkerFaceColor', 'g', 'LineWidth', 1, MarkerEdgeColor = 'green')
    %             else
    %                 errorbar(exp_value.aoa(k), (-exp_value.f_avg(k, lift_dir)), exp_value.f_std(k, 2), 'og', 'HandleVisibility','off', 'CapSize', 18, 'MarkerFaceColor', 'g', 'LineWidth', 1, MarkerEdgeColor = 'green')
    %                 x_vec = [exp_value.aoa(k5), exp_value.aoa(k)];
...
%% Drag: presenting one plot per selected speed, against AoA
    ...
    %                 x_vec = [exp_value.aoa(k1), exp_value.aoa(k)];
    %                 y_vec = [(-exp_value.f_avg(k1, 1))  , (-exp_value.f_avg(k, drag_dir))  ];
    %                 plot(x_vec, y_vec, '--r', 'HandleVisibility','off')
    %             end
    %             k1 = k;
    %          elseif (exp_value.vel(k) == sel_speed(j)) && (exp_value.inflation(k) == sel_inflation(2))
